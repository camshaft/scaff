 ...
 * Defines
 */
var SCAFF_DIR = ".scaff-template";
var SCAFF_CONFIG = ".scaffconfig";

exports.fetch = function fetch(url, directory, done) {
  var args = ["clone", url, join(directory, SCAFF_DIR)]
...
exports.createConfig = function createConfig(url, directory, done) {
  ...
  fs.writeFile(join(directory, SCAFF_CONFIG), JSON.stringify(config, null, "  "), done);
};

exports.open = function open(directory, done) {
  try {
    done(null, require(join(directory, SCAFF_DIR)));
  }
  catch (e) {
    done(e);
  }
...
exports.files = function files(directory, generator, done) {
  ...
  if (typeof generator == "function") {
    ...
    contentPath = join(directory, SCAFF_DIR, "content");
  }
  else {
    contentPath = join(directory, SCAFF_DIR, "generators", generator, "content");
  }
